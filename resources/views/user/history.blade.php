                            ...
                            <table class="table table-striped table-hover table-dark fontMonsseratRegular">
                                ...
                                <tbody>
                                    @foreach ($history as $ok => $item)
                                        <tr>
                                            <th><img src="@if ($item->music->cover_path) {{ asset('storage/' . $item->music->cover_path) }} @else {{ asset('img/now_playing/empty_icon.jpeg') }} @endif"alt="Artist Photo" class="img-fluid rounded-3" style="max-width: 50px; max-height: 50px;"></th>
