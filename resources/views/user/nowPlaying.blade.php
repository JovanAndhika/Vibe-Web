    ...
    <div class="container-fluid vh-100 d-flex flex-column">
        ...
        <div class="row flex-grow-1">
            <div class="col d-flex align-items-center justify-content-center m-3">
                ...
                <img src="@if ($music->cover_path) {{ asset('storage/' . $music->cover_path) }} @else {{ asset('img/now_playing/empty_icon.jpeg') }} @endif" alt="Artist Photo" class="img-fluid rounded-3" style="max-width: 500px; max-height: 500px;">
            </div>
        </div>
        ...
        <div class="row">
            <div class="col d-flex justify-content-center my-3" style="font-size: 30px;">
                ...
                <form action="{{ route( 'user.addlike', ['music' => $music] ) }}" method="post">
                    @method('post')
                    @csrf
                    <button type="submit" class="btn btn-dark" name="addlike" id="addlike"><i class="bi bi-heart-fill text-white"></i></button>
                </form>
            </div>
        </div>
        @elseif(isset($playlist->musics))
        <div class="row d-flex justify-content-center">
            <h1 class="fontMonsseratSemiBold text-center text-warning" style="font-size: 30px;">Playlist : <span class="text-white">{{ $playlist->name }}</span></h1>
        </div>
        {{-- jika ada playlist --}}
        {{-- jika ada --}}
        <div class="row flex-grow-1">
            <div class="col d-flex align-items-center justify-content-center m-3">
                {{-- TODO: tambahkan icon di database music --}}
                <img src="@if ($selected->icon) {{ asset('storage/' . $selected->icon) }} @else {{ asset('img/now_playing/empty_icon.jpeg') }} @endif" alt="Artist Photo" class="img-fluid rounded-3" style="max-width: 300px; max-height: 300px;">
            </div>
        </div>
            ...
            <table class="table table-striped table-hover table-dark fontMonsseratRegular">
                ...
                <tbody>
                    ...
                    <tr>
                        <th>{{ $item->title }}</th>
                        <th>{{ $item->artist }}</th>
                        <th><a href="{{ route('user.nowPlaying') . '/?playlist_id=' . request('playlist_id') . '&index=' . $loop->index }}"><i class="bi bi-play-fill text-white"></i></a></th>
                    </tr>
                    @endforeach
                </tbody>
            </table>
